...
class SimpleAlertPlugin {
	...
	public function register_plugin_scripts_styles() 
	{
		...
		if (!empty($selected_post) && !empty($list_of_posts) && !is_admin() && in_array($post_type,$selected_post) && in_array($post_id,$list_of_posts[$post_type] ) && !is_home())
		{
			...
			wp_enqueue_script('jquery-my', plugin_dir_url( __FILE__ ) . '/js/jquery.js');
			wp_enqueue_style( 'jquery.fancybox', plugin_dir_url( __FILE__ ) . '/css/jquery.fancybox.css');
			wp_enqueue_script('jquery.fancybox', plugin_dir_url( __FILE__ ) . '/js/jquery.fancybox.js');
